...
import io
data = pd.read_csv(io.BytesIO(uploaded['parkinson40.data']))
...
import io
data2 = pd.read_csv(io.BytesIO(uploaded['no_park.data']))
# Dataset is now stored in a Pandas Dataframe
...
from sklearn.preprocessing import MinMaxScaler
scaler = MinMaxScaler((-1, 1))
X_test= scaler.fit_transform(predictor2)
Y_test = target+1
...
print(metrics.accuracy_score(Y_test, y_pred)*10)
